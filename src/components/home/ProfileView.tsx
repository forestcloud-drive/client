      ...
      <div className="bg-white/40 backdrop-blur-xl border border-white/20 rounded-3xl p-8 shadow-2xl">
            ...
            {!profile.hasAccess && (
              <div className="mt-4 p-4 bg-red-50 border border-red-100 rounded-2xl animate-pulse">
                <p className="text-red-700 text-sm font-bold">
                  Unfortunately you don't have access. Contact one of the
                  administrators, or wait for the access.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white/50 p-6 rounded-2xl border border-white/30">
            ...
            <div className="flex items-center space-x-2">
              <div
                className={`w-3 h-3 rounded-full ${profile.hasAccess ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}
              />
              <span
                className={`text-sm font-bold ${profile.hasAccess ? 'text-green-700' : 'text-red-700'}`}
              >
